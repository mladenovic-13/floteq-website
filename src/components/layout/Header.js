import React, { useState } from 'react';
import { Transition } from '@headlessui/react';
import { StaticImage } from 'gatsby-plugin-image';
import Button from '../Button';

function Nav() {
  const [isOpen, setIsOpen] = useState(false);
  return (
    <div>
      <nav className="bg-white shadow w-full">
        <div className="w-full mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex-shrink-0">
              <StaticImage className="h-8 w-32" src="../../images/Logo.png" alt="Workflow" />
            </div>
            <div className="flex items-center">
              <div className="hidden md:block">
                <div className="ml-10 flex items-baseline space-x-4">
                  <a
                    href="#"
                    className=" hover:text-primary-darker  text-primary px-3 py-2 rounded-md text-lg font-medium"
                  >
                    Solutions
                  </a>

                  <a
                    href="#"
                    className="text-primary hover:text-primary-darker px-3 py-2 rounded-md text-lg font-medium"
                  >
                    Pricing
                  </a>

                  <a
                    href="#"
                    className="text-primary  hover:text-primary-darker px-3 py-2 rounded-md text-lg font-medium"
                  >
                    About Us
                  </a>

                  <a
                    href="#"
                    className="text-primary  hover:text-primary-darker px-3 py-2 rounded-md text-lg font-medium"
                  >
                    Contact
                  </a>
                </div>
              </div>
            </div>
            <div>
              <Button className="hidden md:block" size="default">
                Client Login
              </Button>
            </div>
            <div className="-mr-2 flex md:hidden">
              <button
                onClick={() => setIsOpen(!isOpen)}
                type="button"
                className="bg-primary-darker inline-flex items-center justify-center p-2 rounded-md text-white hover:bg-primary-darker focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white"
                aria-controls="mobile-menu"
                aria-expanded="false"
              >
                <span className="sr-only">Open main menu</span>
                {!isOpen ? (
                  <svg
                    className="block h-6 w-6"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    aria-hidden="true"
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      strokeWidth="2"
                      d="M4 6h16M4 12h16M4 18h16"
                    />
                  </svg>
                ) : (
                  <svg
                    className="block h-6 w-6"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    aria-hidden="true"
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      strokeWidth="2"
                      d="M6 18L18 6M6 6l12 12"
                    />
                  </svg>
                )}
              </button>
            </div>
          </div>
        </div>

        <Transition
          show={isOpen}
          enter="transition ease-out duration-200 transform"
          enterFrom="opacity-0 scale-95"
          enterTo="opacity-100 scale-100"
          leave="transition ease-in duration-75 transform"
          leaveFrom="opacity-100 scale-100"
          leaveTo="opacity-0 scale-95"
        >
          {(ref) => (
            <div className="md:hidden" id="mobile-menu">
              <div ref={ref} className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
                <a
                  href="#"
                  className=" text-primary-darker block px-3 py-2 rounded-md text-lg font-medium"
                >
                  Solutions
                </a>

                <a
                  href="#"
                  className="text-primary  hover:text-primary-darker block px-3 py-2 rounded-md text-lg font-medium"
                >
                  Pricing
                </a>

                <a
                  href="#"
                  className="text-primary  hover:text-primary-darker block px-3 py-2 rounded-md text-lg font-medium"
                >
                  About Us
                </a>

                <a
                  href="#"
                  className="text-primary  hover:text-primary-darker block px-3 py-2 rounded-md text-lg font-medium"
                >
                  Contact
                </a>
                <button className=" uppercase mx-auto lg:mx-0 hover:bg-primary-darker bg-primary text-white font-bold rounded-full my-6 py-2 px-6 shadow-lg focus:outline-none focus:shadow-outline transform transition hover:scale-105 duration-200 ease-in-out">
                  Client Login
                </button>
              </div>
            </div>
          )}
        </Transition>
      </nav>
    </div>
  );
}

export default Nav;
